    ...
    <form id="filter-form" action="{{ route('search') }}" method="GET">
        ...
        <div class="mb-6 flex flex-wrap gap-3">
            ...
            >
                ...
                <template x-for="val in selected" :key="val">
                    <span class="flex items-center gap-1 rounded-full bg-indigo-500/20 border border-indigo-500/30 px-2.5 py-1 text-xs text-indigo-300">
                        <span x-text="labelFor(val)"></span>
                        <button type="button" @click="remove(val)" class="hover:text-white ml-0.5 leading-none">×</button>
                    </span>
                </template>
            </div>
